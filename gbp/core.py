...
class VariableNode:
    ...
    def set_prior(self,
                  mean: torch.Tensor,
                  diag_cov: Union[float, torch.Tensor]) -> None:
        """
        Replace the prior N(mean, Σ) associated with this node and
        immediately refresh the local belief.

        Parameters
        ----------
        mean : (d,) torch.Tensor
            New prior mean.
        diag_cov : float or (d,) torch.Tensor
            Diagonal of the covariance matrix (or a scalar σ²).
        """
        if torch.is_tensor(diag_cov) and diag_cov.ndim == 1:
            cov = torch.diag(diag_cov)
        else:  # scalar or full matrix
            cov = diag_cov * torch.eye(self.dofs, dtype=mean.dtype)

        self.prior.set_with_cov_form(mean, cov)
        self.update_belief()
